...
const error = ref('')
const modelInfo = ref<{ name: string; vertices: number; triangles: number; animations: number; ms: number } | null>(null)
...
function loadModel(type: ModelType) {
  ...
  if (type === 'fbx') {
    ...
    loader.load(cfg.url, (obj) => finishLoad(obj, obj.animations, cfg.label, performance.now() - t0), onProgress, onError)
    return
  }
  ...
  loader.load(cfg.url, (gltf) => finishLoad(gltf.scene, gltf.animations, cfg.label, performance.now() - t0), onProgress, onError)
}
  ...
  <div class="flex h-full flex-col gap-3 p-4">
    ...
    <div class="flex flex-wrap items-center gap-2">
      <n-button
        ...
        @click="loadModel(key)"
      >{{ item.label }}</n-button>
      <n-button size="small" :type="activeFeature === 'upload' ? 'primary' : 'default'" @click="triggerUpload">
        上传本地模型
      </n-button>
      <n-button size="small" quaternary @click="clearModel">清除模型</n-button>
      <input ref="fileInputRef" type="file" accept=".glb,.gltf,.fbx" class="hidden" @change="handleFileChange" />
    </div>
    ...
    <div class="flex min-h-0 flex-1 gap-3">
      ...
      <div class="flex w-[420px] shrink-0 flex-col overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm">
        ...
        <pre class="m-0 flex-1 overflow-auto bg-gray-50 p-4 text-[13px] leading-relaxed"><code v-html="code"></code></pre>
        <div class="max-h-[46%] overflow-auto border-t border-gray-100 px-4 py-3 text-[13px] leading-relaxed text-gray-600">
